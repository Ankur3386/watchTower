...
export const signUpUser=async(req:Request,res:Response,next:NextFunction)=>{
  ...
  try {
    ...
    console.log(error)
      return res.status(500).json({error,message:"Internet server error"})
  }
}
...
try {
    ...
    return res.status(200).json({"userId":user.id,"token":token,"username":user.username,"message":"user signedIn successfully"})
} catch (error) {
...
export const getAllApi=async(req:Request,res:Response,next:NextFunction)=>{
console.log("hii1")
const api= await client.project.findMany({
    where:{
        userId:req.id
    },
    select:{
